  ...
  return (
    <div className={className}>
      {socialLinks.map((social) => (
        ...
        >
          <svg
            className="w-4 h-4 fill-current text-gray-300 hover:text-[#FFD45C] transition-colors"
      ...
      <div className="px-4 md:px-8 lg:px-40 py-6 md:py-8">
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between md:gap-10">
          ...
          <div className="flex shrink-0 flex-col gap-3">
            ...
            <SocialLinks className="flex items-center gap-3.5" />
          </div>

          {/* Navigation columns */}
          <div className="grid flex-1 grid-cols-2 gap-x-6 gap-y-5 sm:grid-cols-4 md:gap-x-8 lg:max-w-3xl">
            {footerSections.map((section) => (
              <div key={section.title}>
                <h3 className="mb-2 text-sm font-semibold text-[#FFD45C]">
                  {section.title}
                </h3>
                <ul className="space-y-1">
                  {section.links.map((link) => (
                    <li key={link.name}>
                      <Link
                        href={link.href}
                        className="text-xs text-gray-300 transition-colors duration-200 hover:text-[#FFD45C]"
                      >
                        {link.name}
                      </Link>
                    </li>
        ...
        {/* Bottom bar */}
        <div className="mt-6 flex flex-col items-center justify-between gap-3 border-t border-gray-700 pt-4 text-xs text-gray-400 sm:flex-row">
          <p className="text-center sm:text-left">
            © {new Date().getFullYear()} Vaakya Foundation. All rights reserved.
          </p>

          <div className="flex flex-wrap justify-center gap-x-4 gap-y-1">
            <Link
              href="/privacy-policy"
              className="hover:text-[#FFD45C] transition-colors"
            >
              Privacy Policy
            </Link>
            <Link
